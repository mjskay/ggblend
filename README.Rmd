...
a call to `blend()` to achieve this, there are currently three approaches:
...
An experimental algebraic syntax for combining and applying blends replaces the
function application `|>` with `*` and stacking blends with `^`. For example:

```{r scatter_blend_algebraic}
df |>
  ggplot(aes(x, y, color = set)) +
  list(
